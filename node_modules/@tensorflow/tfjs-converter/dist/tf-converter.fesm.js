        ...
        return tidy(() => {
            ...
            for (let i = 0; i < orderedNodes.length; i++) {
                ...
                if (!tensorsMap[node.name]) {
                    const tensors = executeOp$i(node, tensorsMap, context, this._resourceManager);
    ...
    processStack(inputNodes, stack, context, tensorMap, added, tensorsToKeep, outputNames, intermediateTensorConsumerCount, usedNodes) {
        ...
        while (stack.length > 0) {
            ...
            if (tensorMap[item.node.name] == null) {
                const tensors = executeOp$i(item.node, tensorMap, context, this._resourceManager);
                if (!nodeName) {
                    [nodeName] = getNodeNameAndIndex(item.node.name, context);
                }
